...
hid_t _getFileHandle(std::string filename, void *dfile_ptr)
{
    ...
    if (dfile_ptr)
    {
        DmaFile *dmafile = (DmaFile *) dfile_ptr;
        return dmafile->open(filename.c_str(), false) ? dmafile->file_id : -1;
    }
#endif
...
const unsigned int *cd_values, size_t nbytes, size_t *buf_size, void **buf)
{
    if (flags & H5Z_FLAG_REVERSE)
    {
        ...
        {
            // With the CUDA backend, deviceToHost() allocates a chunk of memory in host
            // memory and performs a memcpy() from device to host; the return value is a
            // pointer to the newly allocated memory chunk.
            ...
            // `output_dataset.data` can only be freed with the CUDA backend -- we end
