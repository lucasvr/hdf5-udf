...
/* Prototype for function implemented on udf_template.cu */
...
bool CudaBackend::run(
    ...
    const json &rules)
{
    /* Due to fork() semantics, this backend cannot run in a sandboxed
     * environment. Here we check if a sandbox is required (as defined
     * in the configuration file) and, if so, abort the execution of
     * the UDF.
     */
    if (rules.contains("sandbox") && rules["sandbox"].get<bool>() == true)
    {
        fprintf(stderr,
            "Error: the CUDA backend does not support sandboxing.\n"
            "Please adjust your settings under %s as appropriate.\n",
            os::configDirectory().c_str());
        return false;
    }
     ...
     * UDF in a separate process due to fork() semantics.
     */
